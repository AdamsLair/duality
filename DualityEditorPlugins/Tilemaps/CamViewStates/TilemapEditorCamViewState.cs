		...
		private static void DrawTileHighlights(Canvas canvas, ICmpTilemapRenderer renderer, Point2 origin, IReadOnlyGrid<bool> highlight, ColorRgba fillTint, ColorRgba outlineTint, TileHighlightMode mode, List<Vector2[]> outlineCache = null)
		{
			...
			canvas.PushState();
			{
				...
				// Fill all highlighted tiles that are currently visible
				{
					...
					// Adjust the Z offset to exceed the topmost visible one as generated by the tilemap
					...
					TileInfo bottomRightTileInfo = 
						...
						tileset.TileData[bottomRightTile.Index] : 
						default(TileInfo);
					int localDepthOffset = bottomRightTile.DepthOffset + bottomRightTileInfo.DepthOffset;
					float maxVisibleDepthOffset = renderer.GetTileDepthOffsetAt(new Point2(
						bottomRightTilePos.X,
						bottomRightTilePos.Y + localDepthOffset + 1));
					canvas.State.ZOffset = maxVisibleDepthOffset - 0.01f;
