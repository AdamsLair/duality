...
namespace FarseerPhysics.Common.Decomposition.Seidel
{
	internal class TrapezoidalMap
	{
		...
		{
			Point max = edges[0].P + this._margin;
			Point min = edges[0].Q - this._margin;

			foreach (Edge e in edges)
			{
				if (e.P.X > max.X) max = new Point(e.P.X + this._margin, max.Y);
				if (e.P.Y > max.Y) max = new Point(max.X, e.P.Y + this._margin);
				if (e.Q.X > max.X) max = new Point(e.Q.X + this._margin, max.Y);
				if (e.Q.Y > max.Y) max = new Point(max.X, e.Q.Y + this._margin);
				if (e.P.X < min.X) min = new Point(e.P.X - this._margin, min.Y);
				if (e.P.Y < min.Y) min = new Point(min.X, e.P.Y - this._margin);
				if (e.Q.X < min.X) min = new Point(e.Q.X - this._margin, min.Y);
				if (e.Q.Y < min.Y) min = new Point(min.X, e.Q.Y - this._margin);
			}
