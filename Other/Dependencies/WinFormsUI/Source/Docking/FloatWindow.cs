	...
	public class FloatWindow : Form, INestedPanesContainer, IDockDragSource
	{
		...
		protected override void WndProc(ref Message m)
		{
			if (m.Msg == (int)Win32.Msgs.WM_NCLBUTTONDOWN)
			{
				...
				IntPtr result = NativeMethods.SendMessage(this.Handle, (uint)Win32.Msgs.WM_NCHITTEST, IntPtr.Zero, m.LParam);
				if (result == (IntPtr)2 && DockPanel.AllowEndUserDocking && this.AllowEndUserDocking)	// HITTEST_CAPTION
				{
					Activate();
            ...
            {
                IntPtr result = NativeMethods.SendMessage(this.Handle, (int)Win32.Msgs.WM_NCHITTEST, IntPtr.Zero, m.LParam);
                if (result == (IntPtr)2)	// HITTEST_CAPTION
                {
                    DockPane theOnlyPane = (VisibleNestedPanes.Count == 1) ? VisibleNestedPanes[0] : null;
